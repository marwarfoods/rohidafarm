...
<head>
    ...
    @if(\App\Models\Setting::get('google_analytics_id'))
        <script async src="https://www.googletagmanager.com/gtag/js?id={{ trim(\App\Models\Setting::get('google_analytics_id')) }}"></script>
        <script>
            window.dataLayer = window.dataLayer || [];
            function gtag(){dataLayer.push(arguments);}
            gtag('js', new Date());
            gtag('config', '{{ trim(\App\Models\Setting::get('google_analytics_id')) }}');
        </script>
    @endif

    {{-- -- Meta Pixel (Facebook Pixel) -- --}}
    @if(\App\Models\Setting::get('meta_pixel_enabled') && (\App\Models\Setting::get('meta_pixel_id') || \App\Models\Setting::get('meta_pixel_code')))
        @if(\App\Models\Setting::get('meta_pixel_id'))
            ...
            <script>
                ...
                fbq('init', '{{ trim(\App\Models\Setting::get('meta_pixel_id')) }}');
                fbq('track', 'PageView');
            </script>
            <noscript>
                <img height="1" width="1" style="display:none" 
                     src="https://www.facebook.com/tr?id={{ trim(\App\Models\Setting::get('meta_pixel_id')) }}&ev=PageView&noscript=1"/>
            </noscript>
            <!-- End Meta Pixel Code -->
        @endif

        @if(\App\Models\Setting::get('meta_pixel_code'))
            {!! \App\Models\Setting::get('meta_pixel_code') !!}
        @endif
    @endif
    ...
    {{-- -- Google Fonts - Outfit (Headings) + Inter (Body & UI) -- --}}
