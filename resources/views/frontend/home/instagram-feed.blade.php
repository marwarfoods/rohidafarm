...
@php
    $instaUrl = \App\Models\Setting::get('social_instagram') ?: 'https://www.instagram.com/rohidafarm/';
    if (Str::contains($instaUrl, 'marwar_foods')) {
        $instaUrl = 'https://www.instagram.com/rohidafarm/';
    }
    ...
    if ($hasRealPosts) {
        ...
        $row2Posts = [
            ['img' => 'https://images.unsplash.com/photo-1500595046743-cd271d694d30?q=80&w=400&auto=format&fit=crop', 'likes' => '4.7k', 'comments' => '320', 'caption' => 'Sunlit morning at Rohida Farm 🌄🌾', 'link' => $instaUrl],
    ...
    // Pad posts to at least 8 items and double for infinite seamless marquee loop
    $makeLoopPosts = function($posts) {
        if (empty($posts)) return [];
        $res = $posts;
        while (count($res) < 8) {
            $res = array_merge($res, $posts);
        }
        return array_merge($res, $res);
    };

    $row1Loop = $makeLoopPosts($row1Posts);
    $row2Loop = $makeLoopPosts($row2Posts);
@endphp
...
<section class="instagram-section py-5 position-relative overflow-hidden"
    ...
    <div class="container py-2 position-relative" style="z-index: 1;">
        ...
        <div class="text-center mb-4 mb-md-5" data-aos="fade-up">
            <a href="{{ $instaUrl }}" target="_blank" rel="noopener noreferrer" class="badge px-3 py-2 rounded-pill font-heading text-uppercase fw-semibold mb-2 text-decoration-none d-inline-flex align-items-center insta-handle-badge" 
                ...
                <i class="bi bi-instagram me-1 fs-6"></i> @rohidafarm
            </a>
            <h2 class="font-heading fw-bold text-dark display-6 m-0">
                Follow Us On <a href="{{ $instaUrl }}" target="_blank" rel="noopener noreferrer" class="text-decoration-none"><span style="background: linear-gradient(45deg, #f09433, #e6683c, #dc2743, #cc2366, #bc1888); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">Instagram</span></a>
            </h2>
        </div>
    </div>
                ...
                @foreach($row1Loop as $post)
                    <div class="insta-marquee-item px-2">
                        <a href="{{ $post['link'] ?? $instaUrl }}" target="_blank" rel="noopener noreferrer" class="insta-square-card d-block position-relative rounded-4 overflow-hidden shadow-sm text-decoration-none group">
                            <img src="{{ $post['img'] }}" class="w-100 h-100 object-fit-cover insta-img" loading="lazy" decoding="async" alt="Rohida Farm Instagram Post">
                            <div class="insta-overlay position-absolute inset-0 d-flex flex-column justify-content-between p-3 text-white">
                                <div class="d-flex justify-content-between align-items-center">
                                    <span class="badge bg-white bg-opacity-25 rounded-pill px-2 py-1" style="font-size: 0.65rem;"><i class="bi bi-instagram me-1"></i> @rohidafarm</span>
                                </div>
