...
<div class="d-flex justify-content-between align-items-center mb-4 pb-2 border-bottom">
    <h1 class="display-6 font-heading fw-bold m-0">
        <i class="bi bi-pencil-square text-primary me-2"></i>Edit: {{ $certification->name }}
    </h1>
    ...
    <form method="POST" action="{{ route('admin.certifications.update', $certification->id) }}" enctype="multipart/form-data">
        ...
        <div class="row g-4">
            ...
            <div class="col-md-6">
                ...
                <input type="text" name="name" class="form-control rounded-3" value="{{ old('name', $certification->name) }}" required>
            </div>

            {{-- Certificate Number --}}
            <div class="col-md-6">
                <label class="form-label fw-bold">Certificate / License Number</label>
                <input type="text" name="certificate_number" class="form-control rounded-3" value="{{ old('certificate_number', $certification->certificate_number) }}">
            </div>

            {{-- Sort Order + Active --}}
            <div class="col-md-4">
                <label class="form-label fw-bold">Sort Order</label>
                <input type="number" name="sort_order" class="form-control rounded-3" value="{{ old('sort_order', $certification->sort_order) }}" min="0">
            </div>
            <div class="col-md-4 d-flex align-items-end">
                <div class="form-check form-switch ps-0 w-100">
                    <label class="form-label fw-bold d-block mb-2">Active on Website</label>
                    <input class="form-check-input" type="checkbox" name="is_active" id="is_active" {{ $certification->is_active ? 'checked' : '' }} style="width: 50px; height: 26px;">
                </div>
            </div>
                    ...
                    <div class="col-md-7">
                        <div class="input-group">
                            <input type="text" name="logo_path" id="logoPathInput" class="form-control media-picker-input" 
                                   placeholder="Select from Media Library or paste image URL..." value="{{ old('logo_path', $certification->logo_path) }}">
                        </div>
                    ...
                    <div class="col-md-5">
                        <div id="logoPreview" class="border rounded-3 p-2 bg-light text-center min-height-80 d-flex align-items-center justify-content-center">
                            @if($certification->logo_path)
                                <img src="{{ asset($certification->logo_path) }}" class="rounded-3 img-fluid border" style="max-height: 120px; object-fit: contain;">
                            @else
                                <span class="text-muted small">No logo set</span>
                            @endif
                        </div>
                    </div>
                </div>
            ...
            <div class="col-12">
                ...
                {{-- Existing Images --}}
                @php $existingImages = $certification->certificate_images ?? []; @endphp
                @if(count($existingImages))
                    <div class="mb-3">
                        <label class="form-label text-muted small fw-semibold">Current Certificate Pages:</label>
                        <div class="d-flex flex-wrap gap-2 p-2 border rounded-3 bg-light">
                            @foreach($existingImages as $img)
                                <img src="{{ asset($img) }}" alt="cert page" style="height: 90px; width: 70px; object-fit: cover; border-radius: 8px; border: 1px solid #ddd;">
                            @endforeach
                        </div>
                        <div class="form-check mt-2">
                            <input class="form-check-input" type="checkbox" name="replace_images" id="replace_images">
                            <label class="form-check-label text-danger fw-bold small" for="replace_images">Replace all existing images with new selection below</label>
                        </div>
                    </div>
                @endif
                ...
                <div class="row g-3 align-items-start">
                    ...
                    <div class="col-md-5">
                        <label class="form-label text-muted small fw-semibold">New Images Preview</label>
        ...
        <div class="mt-4 d-flex gap-2">
            <button type="submit" class="btn btn-primary fw-bold px-4 rounded-3">
                <i class="bi bi-save2-fill me-1"></i> Update Certification
            </button>
