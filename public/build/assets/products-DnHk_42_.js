document.addEventListener(`DOMContentLoaded`,function(){let e=document.getElementById(`productForm`);if(!e)return;let t=parseInt(e.dataset.galleryCount||`0`),n=parseInt(e.dataset.variantCount||`0`),r=document.getElementById(`displayCouponsSelect`);r&&typeof Choices<`u`&&new Choices(r,{removeItemButton:!0,searchEnabled:!0,placeholderValue:`Select coupons to show...`,itemSelectText:``}),window.initMediaPicker&&(initMediaPicker(`#productImageInput`,`#imagePreviewContainer`,`image`,!1),initMediaPicker(`#gallerySelectorInput`,null,`image`,!0));let i={},a=[`ShortDescription`,`Description`,`Benefits`,`Ingredients`];a.forEach(e=>{let t=document.querySelector(`#editor${e}`);t&&typeof ClassicEditor<`u`&&ClassicEditor.create(t).then(t=>{i[e]=t}).catch(e=>console.error(e))}),e.addEventListener(`submit`,function(){a.forEach(e=>{let t=document.getElementById(`editor${e}`);t&&i[e]&&(t.value=i[e].getData())}),h(),S()});let o=document.querySelector(`[name="mrp"]`),s=document.querySelector(`[name="sale_price"]`),c=document.getElementById(`discountBadge`);function l(){if(!o||!s||!c)return;let e=parseFloat(o.value)||0,t=parseFloat(s.value)||0;c.textContent=e>0&&t>0&&t<e?Math.round((e-t)/e*100)+`% off from MRP`:``}o&&s&&(o.addEventListener(`input`,l),s.addEventListener(`input`,l),l());let u=document.getElementById(`btnAddVariant`),d=document.getElementById(`variantsContainer`),f=document.getElementById(`variantGallerySelectorInput`);f||(f=document.createElement(`input`),f.type=`text`,f.id=`variantGallerySelectorInput`,f.style.display=`none`,document.body.appendChild(f),window.initMediaPicker&&initMediaPicker(`#variantGallerySelectorInput`,null,`image`)),f&&f.addEventListener(`change`,function(){this.value});function p(e,t){let n=e.querySelector(`.variant-weight-input`),r=e.querySelector(`.variant-price-input`),i=e.querySelector(`.variant-weight-badge`),a=e.querySelector(`.variant-price-preview`),o=e.querySelector(`.variant-main-img-input`),s=e.querySelector(`.variant-main-img-preview`),c=e.querySelector(`.variant-header-thumb-container`);if(n&&i&&n.addEventListener(`input`,function(){i.textContent=this.value.trim()||`Variant #${t+1}`}),r&&a&&r.addEventListener(`input`,function(){let e=parseFloat(this.value)||0;a.textContent=`₹${e.toLocaleString(`en-IN`)}`}),o){window.initMediaPicker&&!o.parentNode.classList.contains(`media-picker-group`)&&initMediaPicker(o,null,`image`,!1);let e=e=>{if(e){let n=e.startsWith(`http`)||e.startsWith(`/`)?e:`/`+e;s&&(s.innerHTML=`
        ...
        `,m.appendChild(i),t++,i.querySelector(`.btn-remove-gallery-item`).addEventListener(`click`,()=>{i.remove(),h()})}m&&m.querySelectorAll(`.btn-remove-gallery-item`).forEach(e=>{e.addEventListener(`click`,function(){this.closest(`.gallery-item-card`).remove(),h()})});let y=parseInt(e.dataset.faqCount||`0`),b=document.getElementById(`btnAddFaq`),x=document.getElementById(`faqsContainer`);function S(){x&&x.querySelectorAll(`.faq-item-card`).forEach((e,t)=>{let n=e.querySelector(`.faq-question-input`),r=e.querySelector(`.faq-answer-input`),i=e.querySelector(`.faq-sort-order`);n&&(n.name=`faqs[${t}][question]`),r&&(r.name=`faqs[${t}][answer]`),i&&(i.name=`faqs[${t}][sort_order]`,i.value=t)})}function C(e=``,t=``){if(!x)return;document.getElementById(`faqsEmpty`)?.remove();let n=document.createElement(`div`);n.className=`faq-item-card d-flex align-items-start gap-3 border rounded-3 p-3 bg-light`,n.innerHTML=`
            <div class="faq-drag-handle text-muted pt-2" style="cursor: grab;"><i class="bi bi-grip-vertical fs-5"></i></div>
            <div class="flex-grow-1">
                <input type="text" name="faqs[${y}][question]" class="form-control border mb-2 faq-question-input" placeholder="Question" required value="${e}">
                <textarea name="faqs[${y}][answer]" class="form-control border faq-answer-input" rows="2" placeholder="Answer" required>${t}</textarea>
                <input type="hidden" class="faq-sort-order" name="faqs[${y}][sort_order]" value="${y}">
            </div>
            <button type="button" class="btn btn-sm btn-outline-danger border rounded-pill btn-remove-faq"><i class="bi bi-trash"></i></button>
        `,x.appendChild(n),y++,n.querySelector(`.btn-remove-faq`).addEventListener(`click`,()=>{n.remove(),S()})}b&&b.addEventListener(`click`,()=>C()),x&&x.querySelectorAll(`.btn-remove-faq`).forEach(e=>{e.addEventListener(`click`,function(){this.closest(`.faq-item-card`).remove(),S()})}),x&&typeof Sortable<`u`&&Sortable.create(x,{animation:180,ghostClass:`sortable-ghost`,chosenClass:`sortable-chosen`,handle:`.faq-drag-handle`,onEnd:S});let w=document.querySelector(`input[name="name"]`),T=document.querySelector(`input[name="slug"]`);if(w&&T){let e=T.value===``;w.addEventListener(`input`,function(){e&&(T.value=E(this.value))}),T.addEventListener(`input`,function(){e=this.value.trim()===``,this.value=E(this.value)})}function E(e){return e.toString().toLowerCase().replace(/\s+/g,`-`).replace(/[^\w\-]+/g,``).replace(/\-\-+/g,`-`).replace(/^-+/,``).replace(/-+$/,``)}});