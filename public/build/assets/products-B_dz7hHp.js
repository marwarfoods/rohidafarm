document.addEventListener(`DOMContentLoaded`,function(){let e=document.getElementById(`productForm`);if(!e)return;let t=parseInt(e.dataset.galleryCount||`0`),n=parseInt(e.dataset.variantCount||`0`),r=document.getElementById(`displayCouponsSelect`);r&&typeof Choices<`u`&&new Choices(r,{removeItemButton:!0,searchEnabled:!0,placeholderValue:`Select coupons to show...`,itemSelectText:``}),window.initMediaPicker&&(initMediaPicker(`#productImageInput`,`#imagePreviewContainer`,`image`,!1),initMediaPicker(`#gallerySelectorInput`,null,`image`,!0));let i={},a=[`ShortDescription`,`Description`,`Benefits`,`Ingredients`];a.forEach(e=>{let t=document.querySelector(`#editor${e}`);t&&typeof ClassicEditor<`u`&&ClassicEditor.create(t).then(t=>{i[e]=t}).catch(e=>console.error(e))}),e.addEventListener(`submit`,function(){a.forEach(e=>{let t=document.getElementById(`editor${e}`);t&&i[e]&&(t.value=i[e].getData())}),h(),T()});let o=document.querySelector(`[name="mrp"]`),s=document.querySelector(`[name="sale_price"]`),c=document.getElementById(`discountBadge`);function l(){if(!o||!s||!c)return;let e=parseFloat(o.value)||0,t=parseFloat(s.value)||0;c.textContent=e>0&&t>0&&t<e?Math.round((e-t)/e*100)+`% off from MRP`:``}o&&s&&(o.addEventListener(`input`,l),s.addEventListener(`input`,l),l());let u=document.getElementById(`btnAddVariant`),d=document.getElementById(`variantsContainer`),f=document.getElementById(`variantGallerySelectorInput`);f||(f=document.createElement(`input`),f.type=`text`,f.id=`variantGallerySelectorInput`,f.style.display=`none`,document.body.appendChild(f),window.initMediaPicker&&initMediaPicker(`#variantGallerySelectorInput`,null,`image`)),f&&f.addEventListener(`change`,function(){this.value});function p(e,t){let n=e.querySelector(`.variant-weight-input`),r=e.querySelector(`.variant-price-input`),i=e.querySelector(`.variant-weight-badge`),a=e.querySelector(`.variant-price-preview`),o=e.querySelector(`.variant-main-img-input`),s=e.querySelector(`.variant-main-img-preview`),c=e.querySelector(`.variant-header-thumb-container`);if(n&&i&&n.addEventListener(`input`,function(){i.textContent=this.value.trim()||`Variant #${t+1}`}),r&&a&&r.addEventListener(`input`,function(){let e=parseFloat(this.value)||0;a.textContent=`₹${e.toLocaleString(`en-IN`)}`}),o){window.initMediaPicker&&!o.parentNode.classList.contains(`media-picker-group`)&&initMediaPicker(o,null,`image`,!1);let e=e=>{if(e){let n=e.startsWith(`http`)||e.startsWith(`/`)?e:`/`+e;s&&(s.innerHTML=`
                            <img src="${n}" class="w-100 h-100 object-fit-cover rounded">
                        ...
                        `,t()),c&&(c.innerHTML=`<img src="${n}" class="rounded border variant-header-thumb" style="width: 28px; height: 28px; object-fit: cover;">`)}else s&&(s.innerHTML=`<i class="bi bi-image text-muted" style="font-size: 1.3rem;"></i>`),c&&(c.innerHTML=``)},t=()=>{let t=s?.querySelector(`.btn-clear-variant-main-img`);t&&(t.onclick=function(t){t.preventDefault(),t.stopPropagation(),o.value=``,e(``),o.dispatchEvent(new Event(`change`))})};o.addEventListener(`change`,function(){let t=this.value.trim();e(t)}),t()}let l=e.querySelector(`.btn-add-variant-gallery`),u=e.querySelector(`.variant-gallery-chips-container`);l&&u&&l.addEventListener(`click`,function(n){n.preventDefault();let r=e.dataset.index||t;window.openMediaPicker&&window.openMediaPicker({type:`image`,isMultiSelect:!0,onSelect:function(e){!e||!e.length||e.forEach(e=>{let t=e.path,n=document.createElement(`div`);n.className=`position-relative variant-gallery-chip`,n.innerHTML=`
                                    <input type="hidden" name="variants[${r}][gallery_images][]" value="${t}">
                                    <img src="${t.startsWith(`http`)||t.startsWith(`/`)?t:`/`+t}" class="rounded border" style="width: 48px; height: 48px; object-fit: cover;">
            ...
            `,d.appendChild(t),p(t,e),n++}));let m=document.getElementById(`galleryContainer`);m&&typeof Sortable<`u`&&Sortable.create(m,{animation:180,ghostClass:`sortable-ghost`,chosenClass:`sortable-chosen`,handle:`.card`,onEnd:h});function h(){m&&m.querySelectorAll(`.gallery-item-card`).forEach((e,t)=>{let n=e.querySelector(`.gallery-image-path`),r=e.querySelector(`.gallery-video-path`),i=e.querySelector(`.gallery-sort-order`);n&&(n.name=`gallery[${t}][image_path]`),r&&(r.name=`gallery[${t}][video_path]`),i&&(i.name=`gallery[${t}][sort_order]`,i.value=t+1)})}let g=document.getElementById(`gallerySelectorInput`),_=document.getElementById(`btnOpenGalleryPicker`);g&&g.addEventListener(`change`,function(){let e=this.value;e&&(e.split(`,`).forEach(e=>{e&&e.trim()&&v(e.trim())}),this.value=``)}),_&&_.addEventListener(`click`,()=>{if(window.openMediaPicker)window.openMediaPicker({type:`image`,isMultiSelect:!0,onSelect:function(e){!e||!e.length||e.forEach(e=>{e.path&&v(e.path)})}});else{let e=(document.getElementById(`gallerySelectorInput`)?.closest(`.media-picker-group`))?.querySelector(`button`);e&&e.click()}});function v(e,n=``){if(!m)return;let r=document.getElementById(`galleryEmpty`);r&&r.remove();let i=document.createElement(`div`);i.className=`col-6 col-md-4 gallery-item-card`,i.innerHTML=`
        ...
        `,m.appendChild(i),t++,i.querySelector(`.btn-remove-gallery-item`).addEventListener(`click`,()=>{i.remove(),h()})}m&&m.querySelectorAll(`.btn-remove-gallery-item`).forEach(e=>{e.addEventListener(`click`,function(){this.closest(`.gallery-item-card`).remove(),h()})});let y=document.getElementById(`useGlobalFaqsToggle`),b=document.getElementById(`globalFaqNotice`),x=document.getElementById(`customFaqsSection`),S=document.getElementById(`btnAddFaq`),C=document.getElementById(`faqsContainer`),w=parseInt(e.dataset.faqCount||`0`);y&&y.addEventListener(`change`,function(){this.checked?(b?.classList.remove(`d-none`),x?.classList.add(`d-none`),S?.classList.add(`d-none`)):(b?.classList.add(`d-none`),x?.classList.remove(`d-none`),S?.classList.remove(`d-none`))});function T(){C&&C.querySelectorAll(`.faq-item-card`).forEach((e,t)=>{let n=e.querySelector(`.faq-question-input`),r=e.querySelector(`.faq-answer-input`),i=e.querySelector(`.faq-sort-order`);n&&(n.name=`faqs[${t}][question]`),r&&(r.name=`faqs[${t}][answer]`),i&&(i.name=`faqs[${t}][sort_order]`,i.value=t)})}function E(e=``,t=``){if(!C)return;document.getElementById(`faqsEmpty`)?.remove();let n=document.createElement(`div`);n.className=`faq-item-card d-flex align-items-start gap-3 border rounded-3 p-3 bg-light`,n.innerHTML=`
            <div class="faq-drag-handle text-muted pt-2" style="cursor: grab;"><i class="bi bi-grip-vertical fs-5"></i></div>
            <div class="flex-grow-1">
                <input type="text" name="faqs[${w}][question]" class="form-control border mb-2 faq-question-input" placeholder="Question" required value="${e}">
                <textarea name="faqs[${w}][answer]" class="form-control border faq-answer-input" rows="2" placeholder="Answer" required>${t}</textarea>
                <input type="hidden" class="faq-sort-order" name="faqs[${w}][sort_order]" value="${w}">
            </div>
            <button type="button" class="btn btn-sm btn-outline-danger border rounded-pill btn-remove-faq"><i class="bi bi-trash"></i></button>
        `,C.appendChild(n),w++,n.querySelector(`.btn-remove-faq`).addEventListener(`click`,()=>{n.remove(),T()})}S&&S.addEventListener(`click`,()=>E()),C&&C.querySelectorAll(`.btn-remove-faq`).forEach(e=>{e.addEventListener(`click`,function(){this.closest(`.faq-item-card`).remove(),T()})}),C&&typeof Sortable<`u`&&Sortable.create(C,{animation:180,ghostClass:`sortable-ghost`,chosenClass:`sortable-chosen`,handle:`.faq-drag-handle`,onEnd:T});let D=document.querySelector(`input[name="name"]`),O=document.querySelector(`input[name="slug"]`);if(D&&O){let e=O.value===``;D.addEventListener(`input`,function(){e&&(O.value=k(this.value))}),O.addEventListener(`input`,function(){e=this.value.trim()===``,this.value=k(this.value)})}function k(e){return e.toString().toLowerCase().replace(/\s+/g,`-`).replace(/[^\w\-]+/g,``).replace(/\-\-+/g,`-`).replace(/^-+/,``).replace(/-+$/,``)}});